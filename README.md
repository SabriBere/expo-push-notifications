...
IOS_BUNDLE_IDENTIFIER=com.sdemetrio.testnotifications
...
### `npm run workflow:ios:production`

Runs the manual EAS workflow defined in `.eas/workflows/create-ios-production-build.yml`.
...
- `ios.bundleIdentifier` is configured through `IOS_BUNDLE_IDENTIFIER` with `com.sdemetrio.testnotifications` as the default.
...
- The app already requests notification permissions and gets the Expo Push Token in `utils/NotificationsUtils.tsx`.
- A manual iOS production workflow is available at `.eas/workflows/create-ios-production-build.yml`.
